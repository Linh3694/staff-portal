...
const ProjectorProductCard = ({
  ...
  const handleConfirmRecycle = async () => {
    try {
      const response = await axios.put(
        `${API_URL}/projectors/${localProjector._id}/status`,
  ...
  const handleSaveRoom = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/projectors/${localProjector._id}`,
        { room: localRoom._id },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      ...
      const roomResponse = await axios.get(
        `${API_URL}/rooms/${updatedProjector.room}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
          ...
          <div className="flex items-center justify-between bg-gray-100 p-3 rounded-xl mb-4 mt-0 transform transition-transform duration-300 hover:scale-105">
            ...
            <div className="flex items-center space-x-2">
              {editField === "display" ? (
                ...
                <button
                  onClick={() =>
                    handleEditSpec("display", projectorData.specs?.display)
                  }
                >
                  <FiEdit
                    className="text-[#FF5733] hover:scale-110"
          ...
          <div className="flex items-center justify-between bg-gray-100 p-3 rounded-xl mb-4 mt-0 transform transition-transform duration-300 hover:scale-105">
            ...
            <div className="flex items-center space-x-2">
              {editField === "releaseYear" ? (
                ...
                <button
                  onClick={() =>
                    handleEditSpec("releaseYear", projectorData.releaseYear)
                  }
                >
                  <FiEdit
                    className="text-[#FF5733] hover:scale-110"
          ...
          <div className="flex items-center justify-between bg-gray-100 p-3 rounded-xl mt-0 transform transition-transform duration-300 hover:scale-105">
            ...
            <div className="flex items-center space-x-2">
              {editField === "manufacturer" ? (
                ...
                <button
                  onClick={() =>
                    handleEditSpec("manufacturer", projectorData.manufacturer)
                  }
                >
                  <FiEdit
                    className="text-[#FF5733] hover:scale-110"

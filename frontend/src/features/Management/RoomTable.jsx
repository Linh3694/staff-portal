...
import { API_URL, UPLOAD_URL, BASE_URL } from "../../core/config"; // import từ file config
        ...
        const formattedRooms = jsonData.map((row, index) => {
          if (
            ...
            throw new Error(
              `Dòng ${
                index + 2
              } trong file Excel thiếu trường dữ liệu bắt buộc. Vui lòng kiểm tra lại.`
            );
          }
          return {
            name: row["Tên phòng"],
            location: [
              { building: row["Toà nhà"], floor: row["Tầng"].toString() },
            ],

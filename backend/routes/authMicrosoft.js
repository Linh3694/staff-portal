...
router.get("/microsoft/callback", (req, res, next) => {
  ...
  passport.authenticate("azuread-openidconnect", (err, user, info) => {
    if (err) {
        ...
        return res.redirect(`https://360wiser.wellspring.edu.vn/login?error=${encodeURIComponent("Lỗi từ Microsoft OAuth: " + err.message)}`);
      }
      if (!user) {
        ...
        if (info && info.message) {
          ...
          return res.redirect(`https://360wiser.wellspring.edu.vn/login?error=${encodeURIComponent("Authentication failed: " + info.message)}`);
        } else {
          console.error("Không có thông tin cụ thể từ Passport.");
          return res.redirect(`https://360wiser.wellspring.edu.vn/login?error=Authentication+failed:+User+not+found+or+denied+by+Microsoft`);
        }
      }
    try {
      // Tạo JWT token
      ...
      const frontendURL = process.env.FRONTEND_URL || "https://360wiser.wellspring.edu.vn";
      ...
      return res.redirect(`https://360wiser.wellspring.edu.vn/login?error=${encodeURIComponent(tokenError.message)}`);
    }
  })(req, res, next);
});

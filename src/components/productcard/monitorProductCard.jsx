...
const MonitorProductCard = ({ monitorData, onAddRepair, onDeleteRepair, onCloseModal }) => {
  console.log("Props truyền vào MonitorProductCard:", {
    assigned: monitorData.assigned,
  });
  console.log("Assigned Users:", monitorData.assigned);
  console.log("Assigned Users Details:", monitorData.assigned);
  ...
  useEffect(() => {
    if (monitorData.repairs) {
      setRepairs(monitorData.repairs); // Cập nhật danh sách sửa chữa khi component nhận được props mới
    }
  }, [monitorData]);
  ...
  if (!monitorData) {
    return <p>Loading Monitor details...</p>;
      ...
      const assignedUsers = Array.isArray(monitorData.assigned) ? monitorData.assigned : [];
      const repairHistory = Array.isArray(monitorData.repairs) ? monitorData.repairs : [];
      const updateHistory = Array.isArray(monitorData.updates) ? monitorData.updates : [];
    ...
    // Gọi hàm từ component cha để xử lý thêm nhật ký
      ...
      const handleDeleteRepair = (repairId) => {
        ...
        if (onDeleteRepair) {
          // Gọi hàm xóa từ cha
          onDeleteRepair(monitorData._id, repairId)
            .then(() => {
      ...
      <div className="bg-white border border-gray-300 rounded-lg shadow-xl overflow-hidden">
        <div className="p-6">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-[#002147]">{monitorData.name}</h2>
              <p className="text-theme-color-neutral-content">
                Năm sản xuất: {monitorData.releaseYear || "N/A"}
              </p>
            </div>
              ...
              <span
                className={`px-4 py-2 rounded-lg text-xs font-semibold text-center ${
                  monitorData.status === "Active"
                    ...
                    : monitorData.status === "In Repair"
                    ? "bg-yellow-100 text-yellow-800"
                    : monitorData.status === "Lưu kho"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {/* <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{monitorData.specs?.processor || "N/A"}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{monitorData.specs?.ram || "N/A"}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{monitorData.specs?.storage || "N/A"}</p>
                    </div>
                  </div> */}
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{monitorData.specs?.display || "N/A"}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="mb-6 bg-gray-200 p-4 rounded-md">
            ...
            {monitorData.assigned && monitorData.assigned.length > 0 ? (
                  monitorData.assigned.map((user) => (

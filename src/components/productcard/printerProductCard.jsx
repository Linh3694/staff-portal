...
const PrinterProductCard = ({ printerData, onAddRepair, onDeleteRepair, onCloseModal }) => {
  console.log("Props truyền vào PrinterProductCard:", {
    assigned: printerData.assigned,
  });
  console.log("Assigned Users:", printerData.assigned);
  console.log("Assigned Users Details:", printerData.assigned);
  ...
  useEffect(() => {
    if (printerData.repairs) {
      setRepairs(printerData.repairs); // Cập nhật danh sách sửa chữa khi component nhận được props mới
    }
  }, [printerData]);

  if (!printerData) {
    return <p>Loading printer details...</p>;
  }
      // const assignedUsers = Array.isArray(printerData.assigned) ? printerData.assigned : [];
      const repairHistory = Array.isArray(printerData.repairs) ? printerData.repairs : [];
      const updateHistory = Array.isArray(printerData.updates) ? printerData.updates : [];
    ...
    // Gọi hàm từ component cha để xử lý thêm nhật ký
      ...
      const handleDeleteRepair = (repairId) => {
        ...
        if (onDeleteRepair) {
          // Gọi hàm xóa từ cha
          onDeleteRepair(printerData._id, repairId)
            .then(() => {
      ...
      <div className="bg-white border border-gray-300 rounded-lg shadow-xl overflow-hidden">
        <div className="p-6">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-[#002147]">{printerData.name} - {printerData.type}</h2>
              <p className="text-theme-color-neutral-content">
                Năm sản xuất: {printerData.releaseYear || "N/A"}
              </p>
            </div>
              ...
              <span
                className={`px-4 py-2 rounded-lg text-xs font-semibold text-center ${
                  printerData.status === "Active"
                    ...
                    : printerData.status === "In Repair"
                    ? "bg-yellow-100 text-yellow-800"
                    : printerData.status === "Lưu kho"
          ...
          <div className="mb-6 bg-gray-200 p-4 rounded-md">
            ...
            {printerData.assigned && printerData.assigned.length > 0 ? (
                  printerData.assigned.map((user) => (

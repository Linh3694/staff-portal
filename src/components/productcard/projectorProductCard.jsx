...
const ProjectorProductCard = ({ projectorData, onAddRepair, onDeleteRepair, onCloseModal }) => {
  console.log("Props truyền vào projectorProductCard:", {
    assigned: projectorData.assigned,
  });
  console.log("Assigned Users:", projectorData.assigned);
  console.log("Assigned Users Details:", projectorData.assigned);
  ...
  useEffect(() => {
    if (projectorData.repairs) {
      setRepairs(projectorData.repairs); // Cập nhật danh sách sửa chữa khi component nhận được props mới
    }
  }, [projectorData]);

  if (!projectorData) {
    return <p>Loading projector details...</p>;
  }
      // const assignedUsers = Array.isArray(projectorData.assigned) ? projectorData.assigned : [];
      const repairHistory = Array.isArray(projectorData.repairs) ? projectorData.repairs : [];
      const updateHistory = Array.isArray(projectorData.updates) ? projectorData.updates : [];
    ...
    // Gọi hàm từ component cha để xử lý thêm nhật ký
      ...
      const handleDeleteRepair = (repairId) => {
        ...
        if (onDeleteRepair) {
          // Gọi hàm xóa từ cha
          onDeleteRepair(projectorData._id, repairId)
            .then(() => {
      ...
      <div className="bg-white border border-gray-300 rounded-lg shadow-xl overflow-hidden">
        <div className="p-6">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-[#002147]">{projectorData.name} - {projectorData.type}</h2>
              <p className="text-theme-color-neutral-content">
                Năm sản xuất: {projectorData.releaseYear || "N/A"}
              </p>
            </div>
            <div className="px-4 py-2 text-center">
              <span
                className={`px-4 py-2 rounded-lg text-xs font-semibold text-center ${
                  projectorData.status === "Active"
                    ? "bg-green-100 text-green-800"
                    : projectorData.status === "In Repair"
                    ? "bg-yellow-100 text-yellow-800"
                    : projectorData.status === "Lưu kho"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{projectorData.specs?.processor || "N/A"}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{projectorData.specs?.ram || "N/A"}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{projectorData.specs?.storage || "N/A"}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3 bg-gray-200 p-4 rounded-md">
          ...
          <div className="mb-6 bg-gray-200 p-4 rounded-md">
            ...
            {projectorData.assigned && projectorData.assigned.length > 0 ? (
                  projectorData.assigned.map((user) => (

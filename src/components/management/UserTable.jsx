...
const UserTable = ({ handleSyncClients }) => {
  ...
  const confirmDeleteUser = async () => {
    try {
      ...
      setClients((prevClients) =>
        prevClients.filter((client) => client._id !== userIdToDelete)
      );
  ...
  const handleShowProfile = async (userId) => {
    ...
    try {
      ...
      if (!assignedItemsResponse.ok)
        throw new Error("Không thể tải danh sách thiết bị được gán!");
      ...
      setSelectedUser({
        ...userData,
        assignedItems: assignedItemsData,
        tickets: userTickets,
      });
      setIsProfileView(true); // Chuyển sang chế độ xem profile
    } catch (error) {
      console.error(
        "Error fetching user profile or assigned items:",
        error.message
      );
      ...
      const enrichedClients = await Promise.all(
        clients.map(async (client) => {
          try {
            ...
            const items = [
              ...(responseData.items?.laptops || []).map((item) => ({
                ...item,
                type: "Laptop",
              })),
              ...(responseData.items?.monitors || []).map((item) => ({
                ...item,
                type: "Monitor",
              })),
              ...(responseData.items?.printers || []).map((item) => ({
                ...item,
                type: "Printer",
              })),
              ...(responseData.items?.projectors || []).map((item) => ({
                ...item,
                type: "Projector",
              })),
              ...(responseData.items?.tools || []).map((item) => ({
                ...item,
                type: "Tool",
              })),
            ];

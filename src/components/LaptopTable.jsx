          ...
          const fetchRooms = async () => {
            ...
            const handleRevokeLaptop = async (laptopId, reasons) => {
              try {
                ...
                setData((prevData) =>
                  prevData.map((laptop) => (laptop._id === updatedLaptop._id ? updatedLaptop : laptop))
                );
            ...
            const handleUpdateRoom = (updatedLaptop) => {
              ...
              const updatedRoom = rooms.find((room) => room.value === updatedLaptop.room?.value);
            
              const newLaptopData = {
                ...updatedLaptop,
                room: updatedRoom || { value: updatedLaptop.room, label: "Không xác định" },
              };
          ...
          const handleAddLaptop = async (e) => {
            ...
            try {
              ...
              const payload = {
                ...
                assigned: newLaptop.status === "Active" ? newLaptop.assigned.map((user) => user.value) : [],
                    ...
                    const normalizedData = sheetData
                        .map((row, index) => {
                            ...
                            return {
                                ...
                                type: typeof row["Loại (type)"] === "string" ? row["Loại (type)"].trim() : "Laptop", // Mặc định là Laptop
                ...
                <div className="flex items-center justify-start w-full space-x-4 mb-4">   
                     <Dropdown
                        ...
                        <Dropdown
                            button={
                              <button className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-md hover:bg-gray-50 focus:ring-2 focus:ring-[#002147] transform transition-transform duration-300 hover:scale-105">
                                {selectedDepartment === "Tất cả" ? "Phòng ban: Tất cả phòng ban" : `Phòng ban: ${selectedDepartment}`}
                              </button>
                            }
                            children={
                              <div className="flex flex-col gap-2 mt-10 bg-white rounded-lg shadow-lg p-4">
                          ...
                          />
                            <Dropdown
                                button={
                                  <button className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-md hover:bg-gray-50 focus:ring-2 focus:ring-[#002147] transform transition-transform duration-300 hover:scale-105">
                                    {selectedManufacturer === "Tất cả" ? "Nhà sản xuất: Tất cả nhà sản xuất" : `Nhà sản xuất: ${selectedManufacturer}`}
                                  </button>
                                }
                                children={
                                  <div className="flex flex-col gap-2 mt-10 bg-white rounded-lg shadow-lg p-4">
                                    ...
                                    {Array.from(new Set(originalData.map((item) => item.manufacturer))).map((manufacturer) => (
                                      <button
                                        key={manufacturer}
                                        className="text-left px-4 py-2 hover:bg-gray-100 rounded-lg"
                                        onClick={() => {
                              ...
                              <Dropdown
                                  button={
                                    <button className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-md hover:bg-gray-50 focus:ring-2 focus:ring-[#002147] transform transition-transform duration-300 hover:scale-105">
                                      {selectedYear === "Tất cả" ? "Năm sản xuất: Tất cả năm sản xuất" : `Năm sản xuất: ${selectedYear}`}
                                    </button>
                                  }
                                  children={
                                    <div className="flex flex-col gap-2 mt-10 bg-white rounded-lg shadow-lg p-4">
                                      ...
                                      {Array.from(new Set(originalData.map((item) => item.releaseYear))).sort().map((year) => (
                                        <button
                                          key={year}
                                          className="text-left px-4 py-2 hover:bg-gray-100 rounded-lg"
                                          onClick={() => {
                            ...
                            {editingLaptop.status === "Broken" && (
                                <div>
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEditingLaptop({ ...editingLaptop, reason: e.target.value })
                                    }
                                  />
                                </div>
                              )}

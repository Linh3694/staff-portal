...
const PrinterTable = () => {
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((printer) =>
          printer._id === updatedPrinter._id ? updatedPrinter : printer
        )
      );
  ...
  const handleUpdateRoom = (updatedPrinter) => {
    ...
    const updatedRoom = rooms.find(
      (room) => room.value === updatedPrinter.room?.value
    );

    const newPrinterData = {
      ...updatedPrinter,
      room: updatedRoom || {
        value: updatedPrinter.room,
  ...
  const handleClone = async (printer) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/printers`, clonedPrinter, {
        headers: {
  ...
  const handleAddPrinter = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        assigned:
          newPrinter.status === "Active"
            ? newPrinter.assigned.map((user) => user.value)
            : [],
      ...
      try {
        ...
        const normalizedData = sheetData
          .map((row, index) => {
            ...
            const specs = {
              ip:
                typeof row["Bộ Xử Lý (ip)"] === "string"
                  ? row["Bộ Xử Lý (ip)"].trim()
                  : "",
              ram:
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        toast.success(
          `${response.data.addedPrinters} printer(s) đã được thêm thành công!`,
          {
            className: "toast-success",
            progressClassName: "Toastify__progress-bar",
          }
  ...
  const handleExportReport = () => {
    const exportData = (
      ...
      "Tên thiết bị": printer.name,
      Loại: printer.type,
      Serial: printer.serial,
      "Năm sản xuất": printer.releaseYear,
      "Người sử dụng":
        Array.isArray(printer.assigned) && printer.assigned.length > 0
          ? printer.assigned.map((user) => user.label).join(", ")
          : "Chưa bàn giao",
      "Bộ phận của người sử dụng":
        Array.isArray(printer.assigned) && printer.assigned.length > 0
          ? printer.assigned
              .map((user) => user.departmentName || "Không xác định")
              .join(", ")
          : "",
      "Nơi sử dụng": printer.room ? printer.room.label : "Không xác định",
      "Trạng thái": statusLabels[printer.status] || printer.status,
    }));
                ...
                <div className="flex flex-col gap-2 mt-10 bg-white rounded-lg shadow-lg p-4">
                  ...
                  {Array.from(
                    new Set(
                      data.flatMap((item) =>
                        item.assigned?.map(
                          (user) => user.departmentName || "Unknown"
                        )
                      )
                    )
                  ).map((department) => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="Máy in Đen trắng">
                            Máy in Đen trắng
                          </option>
                          <option value="Máy Scan">Máy Scan</option>
                          <option value="Máy Photocopier">
                            Máy Photocopier
                          </option>
                          <option value="Máy đa chức năng">
                            Máy đa chức năng
                          </option>
                        </select>
                      </div>
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-4">
                  Bạn có chắc chắn muốn xóa printer{" "}
                  <strong>{printerToDelete?.name}</strong> không?
                </p>

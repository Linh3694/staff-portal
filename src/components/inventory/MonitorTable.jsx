...
const MonitorTable = () => {
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((monitor) =>
          monitor._id === updatedMonitor._id ? updatedMonitor : monitor
        )
      );
  ...
  const handleUpdateRoom = (updatedMonitor) => {
    ...
    const updatedRoom = rooms.find(
      (room) => room.value === updatedMonitor.room?.value
    );

    const newMonitorData = {
      ...updatedMonitor,
      room: updatedRoom || {
        value: updatedMonitor.room,
  ...
  const handleClone = async (monitor) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/monitors`, clonedMonitor, {
        headers: {
      ...
      const payload = {
        ...
        assigned:
          newMonitor.status === "Active"
            ? newMonitor.assigned.map((user) => user.value)
            : [],
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        toast.success(
          `${response.data.addedMonitors} monitor(s) đã được thêm thành công!`,
          {
            className: "toast-success",
            progressClassName: "Toastify__progress-bar",
          }
  ...
  const handleExportReport = () => {
    const exportData = (
      ...
      "Tên thiết bị": monitor.name,
      Loại: monitor.type,
      Serial: monitor.serial,
      "Năm sản xuất": monitor.releaseYear,
      "Người sử dụng":
        Array.isArray(monitor.assigned) && monitor.assigned.length > 0
          ? monitor.assigned.map((user) => user.label).join(", ")
          : "Chưa bàn giao",
      "Bộ phận của người sử dụng":
        Array.isArray(monitor.assigned) && monitor.assigned.length > 0
          ? monitor.assigned
              ...
              .join(", ")
          : "",
      "Nơi sử dụng": monitor.room ? monitor.room.label : "Không xác định",
      "Trạng thái": statusLabels[monitor.status] || monitor.status,
    }));
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-4">
                  Bạn có chắc chắn muốn xóa monitor{" "}
                  <strong>{monitorToDelete?.name}</strong> không?
                </p>

...
const PrinterProductCard = ({
  ...
  const [editField, setEditField] = useState(null); // Tên trường specs đang chỉnh sửa (ip, ram,...)
  ...
  // Thêm state “localPrinterStatus” để tạm giữ status hiển thị
  const [localStatus, setLocalStatus] = useState(printerData.status);
  const [localPrinter, setLocalPrinter] = useState(printerData);
  ...
  const room = localPrinter.room || null; // Lấy thông tin phòng từ localPrinter
  ...
  useEffect(() => {
    setRefreshKey((prev) => prev + 1); // Tăng giá trị mỗi lần localPrinter thay đổi
  }, [localPrinter]);

  useEffect(() => {
    if (localPrinter?.room) {
      const detailedRoom = rooms.find((room) => room.value === localPrinter.room._id);
      setLocalRoom(detailedRoom || localPrinter.room);
    } else {
      setLocalRoom(null); // Nếu không có room
    }
  }, [localPrinter, rooms, showRoomEditModal,refreshKey]); // Thêm `showRoomEditModal` vào dependency

  useEffect(() => {
    if (printerData?.repairs) {
      setRepairs(printerData.repairs); // Chỉ đồng bộ khi thực sự cần thiết
      setLocalStatus(printerData.status);
    }
  }, [printerData]); // Chỉ chạy khi `printerData` thay đổi
  
  useEffect(() => {
    if (printerData) {
      setLocalPrinter(printerData); // Tránh gọi `setState` khi không cần thiết
      setLocalStatus(printerData.status);
    }
  }, [printerData]); // Chỉ phụ thuộc vào `printerData`

  useEffect(() => {
    if (localStatus) {
      fetchPrinterDetails(localPrinter?._id);
    }
  }, [localStatus]);

  useEffect(() => {
    if (printerData?._id) {
        fetchPrinterDetails(printerData._id);
    }
  }, [printerData?._id]); // Chỉ phụ thuộc vào `_id`
  ...
  useEffect(() => {
    if (printerData?.assignmentHistory?.length > 0) {
      const holder = printerData.assignmentHistory.find(
  ...
  useEffect(() => {
    setLocalPrinter(printerData); // Đồng bộ dữ liệu khi printerData thay đổi
    setLocalStatus(printerData.status); // Đồng bộ trạng thái
  }, [printerData]);
  ...
  useEffect(() => {
    const loadActivities = async () => {
      if (!localPrinter?._id) {
        console.error("Printer ID không hợp lệ:", localPrinter?._id);
        toast.error("Không tìm thấy thông tin thiết bị.");
        return;
      }
  
      try {
        const activities = await fetchActivities("printer", localPrinter._id); // Gọi API với đúng entityType và entityId
...
const handleConfirmAssign = async () => {
  ...
  try {
    ...
    const updatedPrinter = response; // API trả về dữ liệu đã cập nhật
    
    // Cập nhật state
    setLocalPrinter(updatedPrinter); // Đồng bộ dữ liệu cục bộ
    setCurrentHolder({
      user: selectedUser,
      assignedBy: JSON.parse(localStorage.getItem("currentUser")),
      startDate: new Date().toISOString(),
    });
    onUpdatePrinter(updatedPrinter); // Đồng bộ với danh sách cha

    // Gọi lại API để cập nhật dữ liệu chi tiết trong modal
    await fetchPrinterDetails(printerData._id);
  ...
  const previousUsers = printerData.assignmentHistory?.filter(
    (history) => history.endDate);
  ...
  const handleConfirmRecycle = async () => {
    try {
      const response = await axios.put(
        `${API_URL}/printers/${localPrinter._id}/status`,
      ...
      // Cập nhật state
      setLocalPrinter(updatedPrinter);
      setLocalStatus("Standby");
  ...
  const newPrinterData = { ...printerData, assigned: [] };
  ...
  const confirmRevoke = async () => {
    ...
    try {
      ...
      onUpdatePrinter(updatedPrinter); // Đồng bộ với danh sách cha
      await fetchPrinterDetails(localPrinter._id);
...
const handleConfirmBroken = async () => {
  ...
  // -----------------------------------------------------
  // Bấm “chỉnh sửa” (ip / ram / storage / display / releaseYear)
  ...
  const handleSaveSpec = (field, value) => {
    ...
    onUpdateSpecs(printerData._id, payload)
      .then((updatedPrinter) => {
        ...
        setEditField(null);
        setEditValue("");
        setLocalPrinter(updatedPrinter); // Đồng bộ lại dữ liệu
      })
      .catch((error) => {
  ...
  const handleViewDocument = async (filename) => {
    ...
    const fileUrl = `${API_URL}/printers/BBBG/${filename}`;
    const token = localStorage.getItem("authToken");
  ...
  const handleGenerateDocument = async () => {
    try {
      ...
      doc.setData({
        ...
        //// Thông tin printer
        printerName: printerData.name || "Không xác định",
        printerSerial: printerData.serial,
        printerIp: printerData.specs.ip,
        printerRam: printerData.specs.ram,  
        printerStorage: printerData.specs.storage,
        printerreleaseYear: printerData.releaseYear,
  ...
  const handleFileUpload = (e) => {
    ...
    if (!file.name.endsWith(".pdf")) {
      ...
      return;
        ...
        formData.append("printerId", localPrinter._id); // ID printer
        formData.append("userId", currentHolder?.user?._id); // ID người dùng hiện tại
    
        axios
        .post(`${API_URL}/printers/upload`, formData, {
          headers: {
        ...
        .then((response) => {
          ...
          setLocalPrinter(updatedPrinter); // Đồng bộ lại state
          setLocalStatus(updatedPrinter.status); // Cập nhật trạng thái hiển thị
  ...
  const handleSaveRoom = async () => {
    ...
    try {
      // Cập nhật phòng cho printer qua API
      const response = await axios.put(
        `${API_URL}/printers/${localPrinter._id}`,
        { room: localRoom._id },
      ...
      const roomResponse = await axios.get(
        `${API_URL}/rooms/${updatedPrinter.room}`,
        { headers: { Authorization: `Bearer ${localStorage.getItem("authToken")}` } }
      );
      ...
      setLocalPrinter((prev) => ({
        ...prev,
        room: detailedRoom, // Gán phòng chi tiết vào printer
      }));
  ...
  const handleSaveNewActivity = async () => {
    ...
    try {
      const addedActivity = await addActivity({
        ...newActivity,
        entityType: "printer", // Thay đổi khi áp dụng cho entity khác
        entityId: localPrinter._id, // ID thực thể
      });
  ...
  return (
    <div className="max-w-full mx-auto bg-white p-6 rounded-xl shadow-lg">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-2xl font-bold text-[#002147]">{printerData.name}</h2>
          <span
        ...
        <div className="w-44 justify-evenly items-center">
          ...
          <div className="flex items-center justify-between bg-[#f8f8f8] p-3 rounded-xl mb-4 mt-0 transform transition-transform duration-300 hover:scale-105">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm text-theme-color-neutral-content">Địa chỉ IP</p>
                {editField === "ip" ? (
                  <input
                    type="text"
                    value={editValue}
                  ...
                  <p className="font-semibold">{printerData.specs?.ip || "N/A"}</p>
                )}
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {editField === "ip" ? (
                <>
                  <button onClick={() => handleSaveSpec("ip", editValue)}>
                ...
                <button onClick={() => handleEditSpec("ip", printerData.specs?.ip)}>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                {editField === "releaseYear" ? (
                  ...
                  <p className="font-semibold">{printerData.releaseYear || "N/A"}</p>
                )}
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {editField === "releaseYear" ? (
                ...
                <button onClick={() => handleEditSpec("releaseYear", printerData.releaseYear)}>
                  <FiEdit className="text-[#FF5733] hover:scale-110" size={15} />
                </button>
              )}
            </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              {editField === "manufacturer" ? (
                ...
                <p className="font-semibold">{printerData.manufacturer || "N/A"}</p>
              )}
            </div>
          </div>
          <div className="flex items-center space-x-2">
            {editField === "manufacturer" ? (
              ...
              <button onClick={() => handleEditSpec("manufacturer", printerData.manufacturer)}>
                <FiEdit className="text-[#FF5733] hover:scale-110" size={15} />
              </button>
            )}
          </div>
              ...
              {localStatus === "Broken" && (
                <>
                  <div className="bg-[#EA5050] text-red-50 p-4 rounded-xl mb-4">
                    ...
                    <p>- {localPrinter.brokenReason || 'Không rõ lý do'}</p>
                  </div>
                           ...
                           <Dropdown
                             ...
                             children={
                               <div className="flex flex-col gap-3 p-4 rounded-[20px] bg-white shadow-xl shadow-shadow-500 w-[360px] max-h-[400px] overflow-y-auto">
                                 {printerData.assignmentHistory?.length > 0 ? (
                                   printerData.assignmentHistory
...
export default PrinterProductCard;
...
const ToolTable = () => {
  ...
  const handleUpdateRoom = (updatedTool) => {
    ...
    const updatedRoom = rooms.find(
      (room) => room.value === updatedTool.room?.value
    );
  ...
  const handleClone = async (tool) => {
    try {
      ...
      const response = await axios.post(`${API_URL}/tools`, clonedTool, {
        headers: {
      ...
      const payload = {
        ...
        assigned:
          newTool.status === "Active"
            ? newTool.assigned.map((user) => user.value)
            : [],
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        toast.success(
          `${response.data.addedTools} tool(s) đã được thêm thành công!`,
          {
            className: "toast-success",
            progressClassName: "Toastify__progress-bar",
          }
  ...
  const handleExportReport = () => {
    const exportData = (
      ...
      "Tên thiết bị": tool.name,
      Loại: tool.type,
      Serial: tool.serial,
      "Năm sản xuất": tool.releaseYear,
      "Người sử dụng":
        Array.isArray(tool.assigned) && tool.assigned.length > 0
          ? tool.assigned.map((user) => user.label).join(", ")
          : "Chưa bàn giao",
      "Bộ phận của người sử dụng":
        Array.isArray(tool.assigned) && tool.assigned.length > 0
          ? tool.assigned
              ...
              .join(", ")
          : "",
      "Nơi sử dụng": tool.room ? tool.room.label : "Không xác định",
      "Trạng thái": statusLabels[tool.status] || tool.status,
    }));
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-4">
                  ...
                  <strong>{toolToDelete?.name}</strong> không?
                </p>
